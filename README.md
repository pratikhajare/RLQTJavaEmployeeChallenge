...
The EmployeeService communicates with MockEmployeeService, which enforces random rate limiting (allowing only 5–10 requests per 30–60 seconds). 
To ensure resilient integration and avoid long delays or connection exhaustion, we implemented a robust client-side rate-limiting and fallback mechanism.

## Key Features
* Resilience4j RateLimiter:
   Limits outbound requests to 5 per minute (conservative baseline).
   Fails fast if limit is exceeded to avoid blocking threads.

*  Retry with Backoff:
   Retries up to 3 times on 429 Too Many Requests.
   Starts with a 10-second backoff and applies jitter to avoid collision.  

* CircuitBreaker:
    Opens circuit if over 50% of requests fail or slow (>5s).
    Prevents unnecessary calls during downstream unavailability.

* Fallback Handling:
    If all retries fail or circuit is open, a fallback response is returned (e.g., empty or default Employee object).
    Ensures the service responds quickly even during backend pressure.

## Technologies Used
*  Spring Boot WebClient (Reactive)
* Resilience4j: RateLimiter, Retry, and CircuitBreaker
